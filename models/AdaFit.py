...
class VariableScaleLayer(nn.Module):
    def __init__(self, input_scale,output_scale,input_dim,output_dim,need_pre_global=False):
        ...
        # self.bc_1 = BaseBC(192,256)
        # self.bc_2 = BaseBC(448,256)
        # self.feat_bc = BaseBC(128,64)
        # self.fc_1 = BaseBF(128,64)
        # self.fc_2 = BaseBF(256,128)
    def forward(self, x):
        ...
        # global_feature_500 = torch.max(x, 2, keepdim=False)[0]
        # global_feature_500 = self.fc_1(global_feature_500)
        # x = torch.cat([x[:,:,:256], global_feature_500.view(-1, 64, 1).repeat(1, 1, 256)], 1)
        # x = self.bc_1(x)
        # global_feature_256 = torch.max(x, 2, keepdim=False)[0]
        # global_feature_256 = self.fc_2(global_feature_256)
        # x = torch.cat([x[:,:,:128], global_feature_256.view(-1, 128, 1).repeat(1, 1, 128),global_feature_500.view(-1, 64, 1).repeat(1, 1, 128)], 1)
        # x = self.bc_2(x)
        # #pointfeat = self.feat_bc(x)
        # pointfeat = x
        # x = self.bn3(self.conv3(x))
        # global_feature = torch.max(x, 2, keepdim=True)[0]
        # x = global_feature.view(-1, 1024, 1).repeat(1, 1, 128)
        # points = points[:,:,:128]
...
class QSTN(nn.Module):
    def __init__(self, num_scales=1, num_points=500, dim=3, sym_op='max'):
        ...
        self.conv1 = torch.nn.Conv1d(self.dim, 64, 1)
        self.conv2 = torch.nn.Conv1d(64, 128, 1)
        #self.vsl_1 = VariableScaleLayer(500,256,128,256)
        #self.vsl_2 = VariableScaleLayer(256,128,256,128)
        self.vsl_1 = VariableScaleLayer(700,350,128,256)
        self.vsl_2 = VariableScaleLayer(350,175,256,128)
        #self.bc = BaseBC(256,128)
        self.conv3 = torch.nn.Conv1d(128, 1024, 1)
